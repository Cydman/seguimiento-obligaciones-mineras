  ...
  return (
    <div>
      {!open ? (
        ...
        <form action={deactivateObligationAction} className="space-y-3 rounded-xl border border-slate-700 bg-slate-950 p-4">
          <input type="hidden" name="obligation_id" value={obligationId} />

          <p className="text-sm text-slate-300">
            Vas a anular la obligación <span className="font-semibold">{obligationCode}</span>.
          </p>
          ...
          <div className="flex gap-3">
            ...
            <button
              ...
              className="rounded-xl border border-slate-700 px-4 py-3 font-semibold text-white transition hover:border-slate-500"
            >
              Cancelar
            </button>
          </div>
