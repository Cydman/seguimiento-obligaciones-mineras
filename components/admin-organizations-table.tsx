...
export function AdminOrganizationsTable({
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-slate-800 bg-slate-900">
      <div className="grid grid-cols-[1.7fr_1.3fr_1fr_1fr_auto] gap-4 border-b border-slate-800 bg-slate-800/70 px-5 py-4 text-sm font-semibold text-slate-300">
        <div>Nombre</div>
        <div>Documento</div>
        <div>Plan</div>
        <div>Estado</div>
      ...
      {organizations.map((org) => (
        <details key={org.id} className="border-t border-slate-800">
          <summary className="grid cursor-pointer grid-cols-[1.7fr_1.3fr_1fr_1fr_auto] gap-4 px-5 py-4 text-sm text-slate-200 list-none">
            <div className="truncate">{org.name}</div>
            <div>{org.document_number}</div>
            <div className="capitalize">{org.subscription_plan}</div>
